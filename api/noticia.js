  ...
  try {
    ...
    const imgTag = imagen
      ? `<div style="position:relative;overflow:hidden;border-radius:10px;margin-bottom:24px;background:#1e293b;max-height:450px;display:flex;align-items:center;justify-content:center;">
           ...
           <img src="${esc(imagen)}" alt="${esc(titulo)}" class="imagen-principal" style="position:relative;z-index:1;max-height:450px;width:100%;object-fit:contain;" onerror="this.parentElement.style.background='${fallbackImg(categoria)}'">
         </div>`
      : `<img src="${fallbackImg(categoria)}" alt="${esc(titulo)}" class="imagen-principal">`;
...
<head>
  ...
  <meta property="article:section" content="${esc(categoria)}">
  ${imgMeta}
